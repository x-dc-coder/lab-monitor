...
- 验收标准：
  ...
  4. **出口健壮性（R-4）**：label/badge thunk 抛错 → 不白屏且有 console.error 日志；**better-sidebar 被禁用（0.13.0 互斥/整体禁用）时 conversation.view 默认出口正常显示同数据**（降级矩阵生效，docs/architecture/ui-adapters.md）；
...
```
- **并行度**：D-A 与 D-B1 可**并行**（前提：rpc.js 的 snapshot JSON schema 先在 docs/reference/protocol.md 冻结，作为两任务共同契约）；D-C 依赖 P0 两者；D-D 依赖 D-C；D-E 依赖 D-D；**D-B2（better-sidebar 适配器）必须最后**——不进入任何 P0/P1 并行路径（开发顺序：引擎与 Agent 通道优先 → 原生兜底 UI → sidebar 最后）。
...
| 15 | **出口全缺席/全失败（含 slots 缺席）**（② 依赖的 slots 本身也是可选服务；②③④ 全不可用 → 纯核心运行） | 中 | 缓解：核心层完整（P0 验收 5「无任何 UI 出口时核心完整」已覆盖）+ Agent 通道①永远兜底 + 出口各自 try/catch 互不影响；降级矩阵见 docs/architecture/ui-adapters.md（M6） |
...
1. t4 评审（v1.1，§9）+ 用户新架构原则（v1.2，§10）+ t8 复审（v1.3，§11）+ 采样实证（v1.4，§12）后：落地目录骨架 + README + docs/01~05（已由 t9 完成，v1.4 变更同步于本计划）；**docs/reference/data-model.md 先行定稿**：状态机转移表（含 pid 关联链路 T1-1、result 配对规则 T1-2）、单实验跟踪约束（R-2）、buffer 扩容标注（R-3）、**阈值事实来源 = host settings 服务（V2）**；**sampler/ 目录落地**（六文件，t10 §5）；
2. 冻结 docs/reference/protocol.md 的 snapshot JSON schema（含 `platform`/`sources`/`alertsCriticalCount`/`callCount`/`gpu[].degraded`/**`ui.betterSidebarVisible`** 字段与 `thresholds` 请求参数——T2-1/T2-2/T4-2/t6/v1.4；**阈值 last-write-wins 仲裁（M3，host 生效时间戳版）**）；**docs/architecture/ui-adapters.md 定稿**：出口注册策略/优先级/互斥规则/能力降级矩阵；**P0 前最小样例验证**：① cordis_define 装载（T1-3）+ 同名工具重复 defineTool（R-1）；② conversation.view 承载能力（M7）；③ **interop 长驻流增量读实证（v1.4）**；**真实采样实证**：LinuxBackend + WindowsBackend 最小实现跑通，耗时/编码记录进 08-sampling-empirical.md 增补；
...
- `docs/architecture/ui-adapters.md` = 出口层契约（注册策略/优先级/互斥规则/能力降级矩阵）定稿；
...
| L4 时间盒/交付物 | §7 时间盒按 D-B1/D-B2 拆分 + §8 交付物已含 docs/architecture/ui-adapters.md 定稿 |
...
### 12.2 protocol 更新（docs/reference/protocol.md 同步）

- snapshot 响应**加 `platform`（'linux'|'wsl'|'windows-native'）与 `sources`（gpu/cpu/mem/procs 各通道）**——WSL 与 Windows 内存语义不同必须标注（14.3 vs 31.7GB）；`gpu[].degraded` 保留（T1-4）；已同步 `lab-monitor/docs/reference/protocol.md`。
...
全部回填 docs/reference/milestones.md；verify.sh 全绿无回归。
...
- 关键差异与动机：`docs/architecture/core.md` §8-11；迁移设计文档：`docs/research/12-v2-migration.md`。
...
- **A2 多实验并行跟踪**（风险 11 R-2「多轨并存留 v2」）：✅ **2026-08-20 实施完成**——多轨（上限 4 + per-run 判定 + runId 归属）+ 标签分组（TagRule 规则式打标 + lab_ctl tag + tags 聚合 + UI 分组展示）；验证 verify-host [B3]/[E2] 全绿。详见 README V2.3 段。
