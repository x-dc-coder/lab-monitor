...
- **V2.6 跟进（2026-08-22）——「实验历史只存内存」已修复**：V2.5 交付后端到端验证发现重启即失（真实缺陷）→ settings 持久化 `history` 键 + 惰性写回（history[0].endTs 变化触发）+ 启动 restoreEnded 恢复（上限 20）；verify-host 重启模拟新增 3 断言（恢复/runId 精确/summary 完整）。**真实 DSH 实证**：重启后跑短命令 → `settings.yaml` 落盘 `history` 键（run-20260822-001 done 完整 summary）→ 面板「实验历史（1）」展开与持久化数据一致。
- **文档同步**：README V2.5 / V2.6 / docs/usage/usage.md（A1）/ docs/reference/protocol.md 1.4（ended/thresholds/enabled）/ 04-milestones V2.5 + V2.6 完成记录。
