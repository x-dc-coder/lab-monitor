...
| V2.11（2026-09-07） | SDK 对齐 DSH 0.1.2-rc.1：devDeps/peerDeps 全线升级（cordis 4.0.2 / dsh-tools|settings|shell|host-webserver|llm 0.1.2-rc.1 / schemastery 3.18.2 / timer 1.1.4）+ 清理 dsh.client 废弃 inject 声明（dsh-client-runtime 已废弃）+ tsdown external→deps.neverBundle（产物字节级一致）+ dshWorkshop dshVersions 更新；typecheck/build/verify.sh 全绿 |
| V2.12（2026-09-09） | #18 修复：彻底根绝跨会话告警广播打扰——消费 alertTargets 靶向投递 + 发起会话离线且 broadcast=false 严禁广播无关 roots；修复裸解释器指纹模糊匹配误绑常驻服务（Vision-MCP）；优化 io-bottleneck 避免纯 CPU 任务误报；新增 verify-host 回归断言 [B2.8]/[B2.9]/[B2.10] 与真实会话端到端测试方案（docs/research/28） |