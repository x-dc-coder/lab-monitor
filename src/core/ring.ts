...
export function createRing(maxPoints: number, maxMs: number): Ring {
  ...
  return {
    ...
    history(sinceMs: number, bucketMs?: number) {
      ...
      const buckets: { bucket: number; ts: number; gpuUtilSum: number; gpuUtilN: number; gpuMemSum: number; gpuMemN: number; cpuSum: number; cpuN: number; memUsedSum: number; memUsedN: number }[] = []
